<p align="center"><img src="docs/assets/readme-hero.svg" alt="Armsmith — the agent that forges your repo for Arm" width="100%"></p>

# Armsmith

<p align="center">
  <img src="https://img.shields.io/badge/CI-arm64%20%2B%20x86-brightgreen" alt="CI: arm64 + x86 matrix">
  <img src="https://img.shields.io/badge/tests-208%20passing-brightgreen" alt="208 tests passing">
  <a href="LICENSE"><img src="https://img.shields.io/badge/License-MIT-green" alt="License: MIT"></a>
  <img src="https://img.shields.io/badge/Python-3.11%20%7C%203.12-blue" alt="Python 3.11 | 3.12">
  <img src="https://img.shields.io/badge/arch-aarch64%20native-f7941e" alt="aarch64 native">
</p>

<!-- Badges are static facts about this repo (arm64+x86 CI matrix, 208 offline tests,
     MIT, Python 3.11/3.12, aarch64-native). Swap the CI badge for a live
     Actions workflow badge once the public repo is pushed. -->

**The agent that forges your repo for Arm.** Armsmith profiles an AI repo on Arm, diagnoses *why*
...
**Status: hardware-free Phase-1 core.** `208` pytest tests, all green, all offline. Everything in
...
## Quickstart (offline — no Arm hardware, no network beyond pip)

```bash
python3 -m venv .venv && .venv/bin/pip install -e '.[dev]'

.venv/bin/python -m pytest                 # 208 tests, green
.venv/bin/python scripts/verify_offline.py # full loop: scan → gate → sign → verify

.venv/bin/armsmith keys init
.venv/bin/armsmith diagnose --replay fixtures/replays/scenario_ragserve -o report.json
.venv/bin/armsmith verify report.json      # hash + ed25519 + schema + stat recompute
.venv/bin/armsmith rules list
.venv/bin/armsmith rules explain R13
.venv/bin/armsmith doctor --offline --replay fixtures/replays/scenario_ragserve
.venv/bin/armsmith diagnose --replay fixtures/replays/scenario_ragserve --no-sign --pr-dry-run
```
...
tests/                 208 tests (goldens, pos/neg per rule, gate, signing, CLI, e2e)
...
.venv/bin/python -m pytest -q               # 208 tests, ~94% line coverage, all offline
...
| layer | tool | status |
|---|---|---|
| unit + replay suite | pytest (208 tests, ~94% cov) | ✅ green, offline |
| lint | ruff | ✅ gate |
