<div align="center">
  ...
  ![457 tests passing](https://img.shields.io/badge/tests-457%20passing-brightgreen)
...
python -m pytest -q                                       # 457 passing, offline
...
| Quality | pytest + pytest-cov · ruff · mypy · CodeQL · TruffleHog | 457 tests, 100% line coverage |
...
pip install -e '.[dev]'
python -m pytest -q                                                    # 457 passing on aarch64
...
```bash
python -m pytest -q                                            # 457 passing, fully offline
...
.venv/bin/python -m pytest -q               # 457 tests, 100% line coverage
...
| layer | tool | status |
|---|---|---|
| unit + replay suite | pytest (457 tests, 100% cov) | ✅ green, offline |
| lint | ruff | ✅ gate |
...
tests/                 457 tests (goldens, pos/neg per rule, gate, signing, CLI, e2e, live bench)
