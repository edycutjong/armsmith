# Armsmith — PROGRESS (Phase-1 hardware-free core)

Session: 2026-07-04 · build target `build/` · specs FROZEN (deviations → DEVIATIONS.md)
Suite: **208 pytest tests, 208 passing, 0 failing** (`.venv/bin/python -m pytest`) plus
`scripts/verify_offline.py` end-to-end green. No fabricated hardware numbers anywhere — every
measurement-shaped value is a labeled synthetic replay fixture.
...
- **Probe layer** — `Probe` interface; `ReplayProbe` bundle backend (manifest with mandatory
  `synthetic` + provenance labels; loaders refuse unlabeled data); `LiveProbe` stub raising
  TODO(S1). 16 probe kinds mapped.
...
- **ISA-witness (Tier A #1, parser half)** — objdump text parser counting SDOT/UDOT/SMMLA/USMMLA
  + before/after delta narrative, fixture-tested.
...
**208 green now** vs blueprint target **150** (which included hardware-phase tests). The 208 are
all hardware-free (rule pack 49 · benchstats 39 · gate/report/keys 37 · probes/fingerprint/gguf 26 ·
CLI/e2e 24 · evidence/witness/ghpr 22 · planner 11). Hardware-phase tests still OWED at S1+:
...
5. LiveProbe + `scripts/record_replays.sh` (live runs write the same bundle layout ReplayProbe reads).
...
| item | status |
|---|---|
| 1. ISA-witness | parser + delta + fixtures DONE; objdump-on-target at S1 |
